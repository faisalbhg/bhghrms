...
class Jobapplications extends BaseController
{
    ...
    public function get_model_offer()
    {
        ...
        $offerDetails['gridOfferType'] = $postData['gridOfferType'];
        ...
        $ajSignaturePath = 'https://careers.buhaleeba.ae/assets/companies/aj-signature1.png';
        ...
        switch($offerDetails['companyId'])
        {
            case '2':
                $return['offerLetter'] = view('users/offerletter2',$offerDetails);
                break;

            case '5':
                $return['offerLetter'] = view('users/offerletter5',$offerDetails);
                break;
        }
    ...
    public function jobOfffer($companyId,$jobApplicatId,$proposalId)
    {
        ...
        if ($this->request->getMethod() == 'post')
        {
            ...
            {
                case '2':
                    $dompdf->loadHtml(view('users/offerletter2',$offerDetails));
                    break;

                case '4':
                    $dompdf->loadHtml(view('users/offerletter4',$offerDetails));
                    break;

                case '5':
                    $dompdf->loadHtml(view('users/offerletter5',$offerDetails));
                    break;
            }
