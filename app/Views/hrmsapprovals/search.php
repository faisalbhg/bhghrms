...
<head>
	...
	<!-- PAGE TITLE HERE -->
	<title>BHG HRMS::Aprovals Search</title>
        ...
        <div class="content-body">
			...
			<div class="container-fluid">
				<div class="d-flex align-items-center mb-4 flex-wrap">
					<h4 class="fs-20 font-w600  me-auto">Aprovals Search</h4>
					<div>
				...
				<div class="row">
					<div class="col-xl-12">
						<div class="card">
                            ...
                            <?php endif; ?>
                            <?= form_open('approvals-select',array("autocomplete"=>"off")); ?>
                                <?= csrf_field() ?>
								<div class="card-body">
									<div class="col-xl-8">
										<div class="row">
											<div class="col-xl-4  col-md-4 mb-1">
												<label  class="form-label font-w600">Approvals Type<span class="text-danger scale5 ms-2">*</span></label>
												<select class="form-control" name="StageId" id="StageId">
													<option value="">-Select-</option>
													<?php
													foreach($getApprovalMasterList as $getApproval)
													{
													?>
													<option value="<?=$getApproval->StageId;?>"><?=$getApproval->StageTitle;?></option>
													<?php
													}
											...
											<div class="col-xl-12  col-md-12 mb-1" id="divisionResponse">
											</div>
										</div>
									</div>
									<!-- <div>
								...
								<div class="card-footer">
									<div>
										<button type="button" id="clickSearch" class="btn btn-primary" name="forApproval" value="0">Search</button>
									</div>
								</div>
							<?=form_close();?>
						</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('#selectCompany').change(function(){
				$.ajax({
					url: "<?=base_url('ptsn-get-position-grade1-approval');?>",
					type: "get",
					data: {'comapnyId':$('#selectCompany').val()} ,
					success: function (divisionResponse) {
					console.log(divisionResponse);
					$('#divisionResponse').html(divisionResponse.gradeSelectVal);
			...
			{
				$.ajax({
					url: "<?=base_url('get-approvals-list');?>",
					type: "get",
					data: {'companyId':$('#selectCompany').val(),'StageId':$('#StageId').val(),'gradeid':$("input[name='gradeSelect']:checked").val()} ,
					success: function (apprListResponse) {
					console.log(apprListResponse);
					$('#responseAproval').html(apprListResponse.responseAproval);
					//$('#positionDataResponse').html('');
					//$('#departmentDataResponse').html('');
