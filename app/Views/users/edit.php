...
<head>
	...
	<!-- PAGE TITLE HERE -->
	<title>BHG HRMS::Edit HRMS User</title>
        ...
        <div class="content-body">
			...
			<div class="container-fluid">
				<div class="d-flex align-items-center mb-4 flex-wrap">
					<h4 class="fs-20 font-w600  me-auto">Edit HRMS User</h4>
					<div>
				...
				<div class="row">
					<div class="col-xl-12">
						<div class="card">
                            ...
                            <?php endif; ?>
                            <?= form_open('edit-user/'.$user['userId'],array("autocomplete"=>"off")); ?>
                                <?= csrf_field() ?>
								<div class="card-body">
									<div class="col-xl-6">
										<div class="row">
											...
											<div class="col-xl-12  col-md-6 mb-4">
												...
												<input type="text" class="form-control" placeholder="Full Name" aria-label="fullName" name="fullName" value="<?=$user['fullName'];?>">
											...
											<div class="col-xl-12  col-md-6 mb-4">
												...
												<select class="form-control" name="companyId"  id="selectCompany">
													...
													{
													?>
													<option <?php if($user['companyId']==$company['companyId']){ ?> selected <?php } ?> value="<?=$company['companyId'];?>"><?=$company['companyName'];?></option>
													<?php
													}
											...
											<select  class="form-select form-control" name="divisionID" id="divisionID">
												...
												if($divisions)
												{
													...
													{
													  	?>
													  	<option <?php if($user['divisionId']==$division['divisionId']){ ?> selected <?php } ?> value="<?=$division['divisionId'];?>"><?=$division['divisionName'];?></option>
													  	<?php
													}
												}
												?>
											...
											<select onchange="getGradeList();"  class="form-select form-control" name="departmentID" id="department">
												...
												if($departmentList)
												{
													...
													{
													  	?>
													  	<option <?php if($user['departmentId']==$dlvalue['departmentId']){ ?> selected <?php } ?>  value="<?=$dlvalue['departmentId'];?>"><?=$dlvalue['departmentName'];?></option>
													  	<?php
													}
												}
												?>
											...
											<select  class="form-select form-control" name="positionID" id="positionID">
												...
												if($positionList)
												{
													...
													{
													  	?>
													  	<option <?php if($user['roleId']==$position['roleId']){ ?> selected <?php } ?> value="<?=$position['roleId'];?>"><?=$position['roleName'];?></option>
													  	<?php
													}
												}
												?>
										...
										<div class="row">
											...
											<div class="col-xl-12  col-md-6 mb-4">
												...
												<input type="email" class="form-control" placeholder="User Name" aria-label="userName" required name="userName" value="<?=$user['userName'];?>">
										...
										<div class="row">
											...
											<div class="col-xl-12  col-md-6 mb-4">
												...
												<select class="form-control" name="userType"  id="userType">
													<option value="">-Select-</option>
													<option <?php if($user['userType']==1){ ?>selected <?php } ?> value="1">Admin</option>
													<option <?php if($user['userType']==2){ ?>selected <?php } ?> value="2">HR</option>
													<option <?php if($user['userType']==3){ ?>selected <?php } ?> value="3">Management</option>
													<option <?php if($user['userType']==4){ ?>selected <?php } ?> value="4">User</option>
													<option <?php if($user['userType']==5){ ?>selected <?php } ?> value="5">Head</option>
													<option <?php if($user['userType']==6){ ?>selected <?php } ?> value="6">General Manager</option>
												</select>
											...
											<select class="form-select form-control" name="userGroupId" id="userGroupId">
												...
												if($userGroups)
												{
													...
													{
													  	?>
													  	<option <?php if($user['userGroupId']==$userGroup['groupId']){ ?> selected <?php } ?> value="<?=$userGroup['groupId'];?>"><?=$userGroup['groupName'];?></option>
													  	<?php
													}
												}
												?>
